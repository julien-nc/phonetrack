...
class OldPageController extends Controller {
	...
	public function getSessions() {
		...
		while ($row = $res->fetch()) {
			$dbname = $row['name'];
			$dbtoken = $row['token'];
			$sharedWith = $this->sessionService->getUserShares($dbtoken);
			$dbpublicviewtoken = $row['publicviewtoken'];
			$dbpublic = $row['public'];
			$dbautoexport = $row['autoexport'];
			$dbautopurge = $row['autopurge'];
			$dblocked = intval($row['locked']);
			$reservedNames = $this->sessionService->getReservedNames($dbtoken);
			$publicShares = $this->sessionService->getPublicShares($dbtoken);
			$devices = $this->sessionService->getDevices($dbtoken);
			$sessions[] = [
					...
					if ($dbtoken !== null) {
						...
						foreach ($devices as $devid) {
							...
							// we give color (first point given)
								...
								$geofences[$token][$devid] = $this->sessionService->getGeofences($devid);
								// proxims
								...
								$proxims[$token][$devid] = $this->sessionService->getProxims($devid);
							}
	...
	public function importSession(string $path): DataResponse {
		...
		$devices = [];
		if ($done === 1) {
			$devices = $this->sessionService->getDevices($token);
		}
	...
	public function getUserList() {
		return new DataResponse([
			'users' => $this->sessionService->getUserList($this->userId),
		]);
	}
