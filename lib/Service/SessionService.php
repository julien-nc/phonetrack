...
class SessionService {
		...
		if ($userFolder !== null) {
			...
			if ($filePossible) {
				...
				if ($sessionToken !== null) {
					...
					$filterSql = $this->getSqlFilter($filters);
	...
	private function getSqlFilter(?array $filters): string {
		$sql = '';
		if ($filters !== null) {
			$conditions = [];
			foreach (['timestamp', 'altitude', 'accuracy', 'satellites', 'batterylevel', 'speed', 'bearing'] as $column) {
				if (isset($filters[$column])) {
					if (isset($filters[$column]['max'])) {
						$conditions[] = $column . ' <= ' . $this->db_quote_escape_string($filters[$column]['max']);
					}
					if (isset($filters[$column]['min'])) {
						$conditions[] = $column . ' >= ' . $this->db_quote_escape_string($filters[$column]['min']);
					}
				}
			}
