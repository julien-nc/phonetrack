...
export default {
	...
	computed: {
		dataLabels() {
			return {
				timestamps: this.device.points.map(p => p.timestamp),
				traveledDistance: this.getLineDistanceLabels(this.device.points, 0),
			}
		},
		firstValidTimestamp() {
		...
		elevationData() {
			return this.device.points.map(p => p.altitude ?? 0)
		},
		speedData() {
			return this.device.points.map(p => p.speed)
		},
		shouldDrawElevation() {
		...
		shouldDrawChart() {
			return this.shouldDrawSpeed || this.shouldDrawElevation
		},
		chartData() {
